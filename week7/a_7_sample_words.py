...
#clean poem 
clean_poem = ο(poem)
clean_poem_list = clean_poem.split()


def sample_words(sample_size, word_num):

    # checking if the arguments are in correct data type.
    assert type(sample_size) == int, "sample_size should be integer type"
    assert type(word_num) == int, "word_num should be integer type"
...
def word_generator():
    for word in clean_poem_list:
        yield word

if __name__ == '__main__':
    res = sample_words(10,10)
    print(res)